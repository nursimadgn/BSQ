...
int	parse_header(int fd, t_map *map)//header bilgi okuma
{
	...
	map->empty = buf[--i];
	buf[i] = '\0';
	map->rows = ft_atoi(buf);//atoiyi değiştim artık harf gorurse hata sayıp 0 donduruyor
...
t_map   *parse_map(char *filepath)
{
    ...
    if (!fill_grid(fd, map))//gridi mallocluyor mapı kontrol(empty,obstacle) ediyor
    {
        close(fd);
        free_map(map);
        return (NULL);
